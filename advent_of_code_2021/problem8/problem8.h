...
std::vector<std::string> split_into_digits(const std::string& line)
{
	std::vector<std::string> crypts{};
	...
	while (iss >> token) {
		if ("|" == token)
			continue;
		crypts.push_back(token);
	}

	return crypts;
}

int count_overlaps(const std::string& crypt1, const std::string& crypt2)
{
	int count{ 0 };
	for (auto& c1 : crypt1) {
		for (auto& c2 : crypt2) {
...
std::string find_one(const std::string& line)
{
	for (auto& crypt : split_into_digits(line)) {
		if (crypt.length() == 2) {
			return crypt;
		}
	}
...
std::string find_four(const std::string& line)
{
	for (auto& crypt : split_into_digits(line)) {
		if (crypt.length() == 4) {
			return crypt;
...
std::string find_seven(const std::string& line)
{
	for (auto& crypt : split_into_digits(line)) {
		if (crypt.length() == 3) {
			return crypt;
...
std::string find_eight(const std::string& line)
{
	for (auto& crypt : split_into_digits(line)) {
		if (crypt.length() == 7) {
			return crypt;
...
char six_nine_or_zero(std::string crypt, std::string one, std::string four)
{
	int overlaps_with_one{ count_overlaps(crypt, one) };
	int overlaps_with_four{ count_overlaps(crypt, four) };

	if (overlaps_with_one == 1 && overlaps_with_four == 3)
		return '6';
	if (overlaps_with_one == 2 && overlaps_with_four == 4)
		return '9';
	return '0';
}

char two_three_or_five(std::string crypt, std::string one, std::string four)
{
	int overlaps_with_one{ count_overlaps(crypt, one) };
	int overlaps_with_four{ count_overlaps(crypt, four) };

	if (overlaps_with_one == 1 && overlaps_with_four == 2)
		return '2';
	if (overlaps_with_one == 2 && overlaps_with_four == 3)
		return '3';
	return '5';
}
...
void part1(const std::vector<std::string>& lines)
{
	int count{ 0 };
	for (auto& line : lines) {
		std::vector<std::string> output_digits{ split_into_digits(line) };
		for (auto i = output_digits.begin() + 10; i != output_digits.end(); ++i) {
			switch ((*i).length()) {
			case 2:
			case 3:
			case 4:
...
void part2(const std::vector<std::string>& lines)
{
	...
	for (auto& line : lines) {
		std::string num{};
		std::string one{ find_one(line) };
		std::string four{ find_four(line) };
		std::string seven{ find_seven(line) };
		std::string eight{ find_eight(line) };

		std::vector<std::string> output_digits{ split_into_digits(line) };
		for (auto i = output_digits.begin() + 10; i != output_digits.end(); ++i) {
			if ((*i).length() == 5)
				num += two_three_or_five(*i, one, four);
			else if ((*i).length() == 6)
				num += six_nine_or_zero(*i, one, four);
			else if ((*i).length() == count_overlaps(*i, one))
				num += '1';
			else if ((*i).length() == count_overlaps(*i, four))
				num += '4';
			else if ((*i).length() == count_overlaps(*i, seven))
				num += '7';
			else if ((*i).length() == count_overlaps(*i, eight))
				num += '8';
		}
		sum += std::stoi(num);
	}
